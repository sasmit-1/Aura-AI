...
function handleMilestoneVerified(data) {
  ...
  addActivityLog(
    `🔔 ORACLE: Milestone ${milestoneId || ""} verified for project #${projectId} — escrow DISBURSED`,
  );
  ...
  if (card) {
    ...
    if (btn) {
      btn.disabled = true;
      btn.className =
        "deploy-btn w-full py-3 bg-green-500/10 border border-green-500/30 text-green-400 font-medium rounded-xl cursor-default text-sm";
      ...
      if (btnContainer && !btnContainer.querySelector(".verified-source-tag")) {
        ...
        tag.className =
          "verified-source-tag text-center text-[10px] text-emerald-400/70 font-mono mt-2";
...
function initRadarChart(project) {
    if (radarChartInstance) {
        radarChartInstance.destroy();
    }
    
    const container = document.getElementById('radar-chart-container');
    const canvas = document.getElementById('projectRadarChart');
    if (!container || !canvas) return;

    const aiScore = project.ai_feasibility_score || 0;
    const ipScore = project.ip_defensibility_score || 0;
    const esgScore = parseInt(project.esg_impact_score) || 0;
    const trlMatch = String(project.technical_readiness_level || "0").match(/(\d+)/);
    const trlVal = trlMatch ? parseInt(trlMatch[1]) : 0;
    const scaledTrl = Math.min(Math.round(trlVal * 11.1), 100);

    const isHighRisk = aiScore < 50 || (project.red_flag_warnings && project.red_flag_warnings.length > 0);
    const bgColor = isHighRisk ? 'rgba(239, 68, 68, 0.4)' : 'rgba(16, 185, 129, 0.4)';
    const borderColor = isHighRisk ? '#ef4444' : '#10b981';

    radarChartInstance = new Chart(canvas, {
        type: 'radar',
        data: {
            labels: ['Feasibility', 'IP Defensibility', 'ESG Impact', 'Tech Readiness'],
            datasets: [{
                data: [aiScore, ipScore, esgScore, scaledTrl],
...
function openProjectModal(projectId) {
  ...
  document.getElementById("modal-linkedin").textContent =
    project.linkedin || "Not provided";
  ...
  document.getElementById("modal-tam").textContent =
    project.market_tam_estimate || "N/A";
  document.getElementById("modal-trl").textContent =
    project.technical_readiness_level
      ? `TRL ${project.technical_readiness_level}/9`
      : "N/A";
  document.getElementById("modal-esg").textContent = project.esg_impact_score
    ? `${project.esg_impact_score}/100`
    : "N/A";
  ...
  strengthsList.innerHTML = (project.key_strengths || [])
    .map(
      (s) =>
  ...
  risksList.innerHTML = (project.critical_risks || [])
    .map(
      (r) =>
    ...
    // IP Collision Risk
    const ipCollisionEl = document.getElementById("modal-ip-collision");
    if (ipCollisionEl) {
        ipCollisionEl.textContent = project.ip_collision_risk ? `⚠ ${project.ip_collision_risk}` : "";
    }

  // Cyber Vulnerabilities (Deep-Tech panel)
  const vulnsList = document.getElementById("modal-vulns");
  if (vulnsList) {
    vulnsList.innerHTML = (project.cyber_vulnerabilities || [])
      .map(
        (v) =>
  ...
  // BMS Threat Model (Cyber Audit)
  const cyberThreatContainer = document.getElementById("cyber-threat-container");
  const cyberThreatsList = document.getElementById("modal-cyber-threats");
  if (cyberThreatContainer && cyberThreatsList) {
    const cyberVulns = project.cyber_vulnerabilities || [];
    if (cyberVulns.length > 0) {
      cyberThreatContainer.classList.remove("hidden");
      cyberThreatsList.innerHTML = cyberVulns
        .map(
          (v) =>
            `<li class="flex items-start gap-2"><span class="text-cyan-400 mt-0.5">🛡</span><span class="text-gray-300 text-sm">${escapeHtml(v)}</span></li>`,
        )
        .join("");
    } else {
      cyberThreatContainer.classList.add("hidden");
      cyberThreatsList.innerHTML = "";
    }
  }
  ...
  if (compsContainer) {
    compsContainer.innerHTML = (project.competitor_landscape || [])
      .map(
        (c) =>
  ...
  typeWriter(
    summaryEl,
    project.scientific_summary || "No summary available.",
    5,
  );
  if (milestoneEl) {
    setTimeout(() => {
      typeWriter(
        milestoneEl,
        project.smart_milestone || "No milestone defined.",
        8,
      );
    }, 150);
  }
  ...
  if (chatWindow && chatInput) {
      chatWindow.innerHTML = `<div class="text-aura-dim">[SYSTEM] Ask a question about this company's architecture...</div>`;
      chatInput.value = "";
  }
...
function closeProjectModal() {
  ...
  // Destroy Radar Chart if needed (handled in init Radar Chart, but good for cleanup)
  if (radarChartInstance) {
      radarChartInstance.destroy();
...
async function deployCapital(projectId) {
  ...
  if (btn) {
    ...
    btn.className =
      "deploy-btn w-full py-3 bg-[#ffb800]/10 border border-[#ffb800]/30 text-[#ffb800] font-medium rounded-xl text-sm opacity-60 cursor-wait";
  }
  ...
  setTimeout(() => {
    if (btn) {
      ...
      btn.className =
        "deploy-btn w-full py-3 bg-[#ffb800]/5 border border-[#ffb800]/20 text-[#ffb800]/70 font-medium rounded-xl text-sm cursor-default";
    }
    addActivityLog(
      `Capital locked in escrow for project #${projectId} — waiting for oracle webhook`,
    );
  }, 1500);
}
...
function initFounderUpload() {
  ...
  function showTerminalSequence() {
    ...
    if (!terminal) {
      ...
      terminal.className =
        "mt-6 bg-black border border-aura-green/30 rounded-2xl p-6 font-mono text-sm";
      terminal.style.boxShadow =
        "0 0 40px rgba(0,255,136,0.08), inset 0 0 60px rgba(0,0,0,0.5)";
      ...
      setTimeout(
        () => {
          ...
          if (idx === terminalLines.length - 1) {
            ...
            cursor.className =
              "inline-block w-2 h-4 bg-aura-green ml-1 animate-pulse";
  ...
  form.addEventListener("submit", async (e) => {
    ...
    if (!fileInput.files.length) {
      ...
      if (!existingError) {
        ...
        errorMsg.className =
          "upload-error text-[#ff3b5c] text-sm mt-2 font-medium";
    ...
    try {
      ...
      formData.append(
        "founder_name",
        document.getElementById("company-name")?.value || "Demo Founder",
      );
      formData.append(
        "milestone_desc",
        document.getElementById("milestone-desc")?.value ||
          "Phase 1: Lab Prototype Verification",
      );
      formData.append(
        "funding_amount",
        parseFloat(
          (document.getElementById("funding")?.value || "500000").replace(
            /[,$]/g,
            "",
          ),
        ) || 500000,
  ...
  function showResult(project) {
    ...
    const metrics = [
      ...
      },
      {
        ...
        value: project.capex_estimate || "N/A",
        color: "text-cyan-400",
      },
      {
        label: "Target Efficiency",
        value: project.target_efficiency || "N/A",
        color: "text-cyan-400",
      },
      {
        label: "AI Feasibility Score",
        value: `${project.ai_feasibility_score}/100`,
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Deal Flow Slider Logic
  const slider = document.getElementById("feasibility-slider");
  const sliderLabel = document.getElementById("feasibility-label");
  ...
  // Ask Aura Chat Logic
  const chatInput = document.getElementById("aura-chat-input");
  const chatSend = document.getElementById("aura-chat-send");
  const chatWindow = document.getElementById("aura-chat-window");

  if (chatInput && chatSend && chatWindow) {
    const sendMessage = async () => {
      const text = chatInput.value.trim();
      if (!text || !_currentModalProjectId) return;

      // Append user msg
      const userMsg = document.createElement("p");
      userMsg.className = "text-white text-right mb-2";
      ...
      aiMsg.className = "text-aura-green mb-2";
      aiMsg.textContent = "Aura Engine is thinking...";
      chatWindow.appendChild(aiMsg);
      chatWindow.scrollTop = chatWindow.scrollHeight;

      try {
        const resp = await fetch(`${API_BASE}/api/ask_aura`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ project_id: _currentModalProjectId, question: text })
        });
        const data = await resp.json();
        if (resp.ok) {
           typeWriter(aiMsg, data.answer, 15);
        } else {
           aiMsg.textContent = `Error: ${data.detail || "Query failed"}`;
           aiMsg.classList.add("text-red-400");
        }
      } catch (e) {
        aiMsg.textContent = "Error connecting to Aura API.";
        aiMsg.classList.add("text-red-400");
      }
    };
